...
/**
 * Casillero que, al pasar o caer en él, varía su comportamiento dependiendo de si en él se encuentra o no la estrella
 * Si la estrella está en el casillero cuando el jugador pasa o cae, se le pregunta si quiere comprar la estrella por 30 monedas.
 * Si la estrella no está en el casillero, se deja al jugador en la posición siguiente, ahorrándole un movimiento.
 * @author facundotourn
 *
 */
public class EstrellaCasillero extends Casillero {
	...
	public EstrellaCasillero(int id, SiguienteCasillero[] next, Tablero tablero) {
		super(id, next, true);
	...
	public void efecto(Jugador jugador) {
		...
		if (tieneEstrella) {
			if (jugador.getMonedas() > PRECIO_ESTRELLA) {
				...
				System.out.println(jugador.getUser().getNombre() + " tiene " + jugador.getMonedas() + " MONEDAS, pero la estrella tiene un precio de " + PRECIO_ESTRELLA + " MONEDAS.");
			}
		} else {
...
//		}
	...
	private void venderEstrella(Jugador jugador) {
		jugador.setMonedas(-PRECIO_ESTRELLA);
